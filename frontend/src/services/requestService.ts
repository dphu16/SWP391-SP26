...
export interface RequestRecord {
  requestId: string;
  employeeId: string;
  requestType: "LEAVE" | "OT" | "SHIFT_CHANGE" | "OTHER";
  reason: string;
  startDate: string;
  endDate: string;
...
export interface RequestResponseDTO {
  ...
  employeeName: string;
  deptName: string;
  requestType: "LEAVE" | "OT" | "SHIFT_CHANGE" | "OTHER" | "APPROVAL";
  status: "PENDING" | "APPROVED" | "REJECTED";
  reason: string;
  startDate: string;
  endDate: string;
...
export const getAllRequestsForReview = async (): Promise<
  RequestResponseDTO[]
> => {
...
export const approveRequest = async (
  ...
  const response = await apiClient.put(`/api/v1/requests/${id}/approve`, {
    managerComment,
  });
  return response.data;
};
...
export const rejectRequest = async (
  ...
  const response = await apiClient.put(`/api/v1/requests/${id}/reject`, {
    managerComment,
  });
  return response.data;
};
