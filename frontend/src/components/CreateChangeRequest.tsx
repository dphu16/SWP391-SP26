...
const FieldWrapper: React.FC<FieldWrapperProps> = ({ id, label, required, error, children, hint }) => (
 ...
 <label htmlFor={id} className="text-sm font-semibold text-text-primary-light select-none">
 {label}
 {required && <span className="text-rose-500 ml-0.5" aria-hidden="true">*</span>}
 </label>
 {children}
 {hint && !error && (
 <p className="text-xs text-text-secondary-light ">{hint}</p>
 )}
 {error && (
...
const inputNormalBorder = "border-border-light focus:border-primary focus:ring-2 focus:ring-primary/20";
...
const CreateChangeRequest: React.FC = () => {
 ...
 <div className="bg-surface-light rounded-2xl border border-border-light p-6 space-y-5 shadow-xs">
 ...
 <h2 className="text-base font-heading font-semibold text-text-primary-light ">
 Request Information
 </h2>
 </div>
 ...
 {/* Dynamic fields card */}
 {form.requestType && (
 <div className="bg-surface-light rounded-2xl border border-border-light p-6 space-y-5 shadow-xs animate-fade-in">
 <div className="flex items-center gap-2 mb-1">
