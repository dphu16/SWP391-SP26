...
const SkeletonRow: React.FC = () => (
  <tr className="border-b border-gray-100 ">
    ...
    <td className="px-4 py-4 text-center">
      <div className="skeleton h-7 w-28 rounded-lg mx-auto" />
    </td>
  </tr>
);
...
const OffboardingTrackerModal: React.FC<{
  ...
  const steps = [
    {
      ...
      description:
        "Gửi thông báo offboarding chính thức qua email và hệ thống.",
      icon: (
        <svg
          viewBox="0 0 24 24"
          fill="none"
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl animate-fade-in">
              ...
              {steps.map((step) => {
                ...
                return (
                  <div key={step.id} className="flex gap-6 relative group">
                    ...
                    {/* Content */}
                    <div
                      className={`flex-1 pt-2 transition-opacity duration-300 ${!isCurrent && !isCompleted ? "opacity-50 group-hover:opacity-80" : ""}`}
                    >
                      <div className="flex items-center justify-between">
                        <h3
                          className={`text-[15px] font-bold ${isCurrent ? "text-primary" : "text-text-primary-light"}`}
                        >
        ...
        <div className="px-6 py-5 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between">
          <div className="flex items-center gap-2 text-text-muted-light">
            ...
            <span className="text-xs font-medium">
              Quá trình này có thể mất vài ngày làm việc.
            </span>
          </div>
...
const EmployeeOffboarding: React.FC = () => {
  ...
  const [trackingEmployee, setTrackingEmployee] =
    useState<OffboardingEmployee | null>(null);

  const handleImmediateStatusChange = () => {
    if (
      window.confirm(
        "Bạn có chắc chắn muốn chuyển trạng thái nhân viên này ngay lập tức không?",
      )
    ) {
      // Logic call API here...
    ...
    async (_pageNum: number) => {
      try {
        ...
        const res = await apiClient.get<
          OffboardingEmployee[] | PageResponse<OffboardingEmployee>
        >(API_URL);
  ...
  const handleReactivate = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to reactivate ${selectedIds.size} selected employee(s)?`,
      )
    )
      return;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-text-secondary-light text-sm">
          Handle the offboarding process and handover tasks for departing
          employees.
        </p>
      </div>
          ...
          <div className="py-20 flex flex-col items-center gap-3 text-center animate-fade-in">
            <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center">
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M10 2a8 8 0 100 16A8 8 0 0010 2zm0 14a6 6 0 110-12 6 6 0 010 12zm0-9a1 1 0 00-1 1v3a1 1 0 002 0V8a1 1 0 00-1-1zm0 6a1 1 0 100-2 1 1 0 000 2z"
                  clipRule="evenodd"
                />
              </svg>
            </div>
