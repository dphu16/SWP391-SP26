...
function buildStats(employees: Employee[]): DashboardStats {
  ...
  return {
    total: all.length,
    active: all.filter((e) => e.statusEmp?.toUpperCase() === "OFFICIAL").length,
    onboarding: all.filter((e) => e.statusEmp?.toUpperCase() === "INTERN")
      .length,
    onLeave: 0, // Not matching a direct status in EmployeeStatus
    probation: all.filter((e) => e.statusEmp?.toUpperCase() === "PROBATION")
      .length,
    inactive: all.filter((e) => ["TERMINATED", "RESIGNED"].includes(e.statusEmp?.toUpperCase() || ""))
      .length,
  };
}
...
// ─── Avatar ───────────────────────────────────────────────────────────────────
const Avatar: React.FC<{ name: string; url?: string; size?: string }> = ({
  name,
  url,
  size = "w-9 h-9",
}) => {
  ...
  return (
    <div
      className={`${size} rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${color}`}
    >
      {initials}
    </div>
  );
...
const STATUS_CFG: Record<string, { dot: string; text: string; bg: string }> = {
  ...
  TERMINATED: { dot: "bg-red-500", text: "text-red-700 ", bg: "bg-red-50 " },
  RESIGNED: { dot: "bg-orange-500", text: "text-orange-700 ", bg: "bg-orange-50 " },
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold ${cfg.text} ${cfg.bg}`}
    >
            ...
            <thead>
              <tr className="border-b border-gray-100 ">
                {[
                  ...
                  <th
                    key={h}
                    className="px-6 py-3 text-[11px] font-semibold uppercase tracking-wider text-text-secondary-light "
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-gray-50 text-sm">
              {loading ? (
                  ...
                  <tr key={i} className="border-b border-gray-100 ">
                    {Array.from({ length: 6 }).map((__, j) => (
                      <td key={j} className="px-6 py-4">
                        <div
                          className={`skeleton rounded h-4 ${j === 0 ? "w-36" : j === 5 ? "w-8" : "w-24"}`}
                        />
                      </td>
                    ))}
                  </tr>
                ...
                employees.map((emp) => (
                  <tr
                    key={emp.id}
                    className="group hover:bg-gray-50/80 table-row-hover"
                  >
                    ...
                    <td className="px-6 py-3.5">
                      <StatusBadge status={emp.statusEmp} />
                    </td>
                    <td className="px-6 py-3.5">
                      <button
