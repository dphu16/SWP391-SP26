...
const EmployeeTable: React.FC<EmployeeTableProps> = ({
  ...
  const fetchEmployees = useCallback(
    async (pageNum: number, currentSearch?: string) => {
      try {
        ...
        let params: any = { page: pageNum, size: 10 };
  ...
  const SortIcon: React.FC<{ field: string }> = ({ field }) => (
    <svg
      viewBox="0 0 16 16"
      fill="currentColor"
      className={`w-3.5 h-3.5 transition-colors ${sortField === field ? "text-primary" : "text-gray-300 group-hover:text-gray-400"}`}
    >
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-center sticky right-0 bg-surface-light">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button
                            onClick={() => handleDeactivateSingle(emp)}
