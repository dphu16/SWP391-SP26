...
export default function AIChatBox() {
  ...
  const [headerElementsMounted, setHeaderElementsMounted] = useState(false);

  useEffect(() => {
    setHeaderElementsMounted(true);
  }, []);
  ...
  const headerLeftSlot = headerElementsMounted ? document.getElementById("ai-chat-header-left") : null;
  const headerRightSlot = headerElementsMounted ? document.getElementById("ai-chat-header-right") : null;
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-full w-full bg-surface-light" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
        ...
        {/* ── Teleported Header Content ── */}
        {headerLeftSlot && createPortal(
        ...
        {headerRightSlot && createPortal(
          <button
            onClick={clearHistory}
            title="Xóa lịch sử trò chuyện"
            className="p-2 rounded-lg text-text-secondary-light hover:text-rose-600 hover:bg-rose-50 transition-colors duration-200 cursor-pointer animate-fade-in"
          >
            <TrashIcon />
          </button>,
          headerRightSlot
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* Chat Pane */}
          <div className={`flex flex-col overflow-hidden transition-all duration-300 ${(activeFileBase64 && showPdf) ? "w-1/2 border-r border-border-light" : "w-full"}`}>
            ...
            <div className="flex items-end gap-2 px-4 py-3 border-t border-border-light bg-surface-light flex-shrink-0">
              ...
              {activeFileBase64 && !showPdf && (
                <button
                  onClick={() => setShowPdf(true)}
                  title="Mở PDF hợp đồng"
                  className="w-9 h-9 rounded-xl border border-primary text-primary flex items-center justify-center flex-shrink-0 hover:bg-primary/10 transition-colors cursor-pointer animate-fade-in"
                >
                  <FileTextIcon />
                </button>
              )}
