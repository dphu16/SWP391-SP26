...
type ModalType = "Leave" | "OT" | "Other" | "Resignation" | "PersonnelChange" | null;
...
// Cấu hình Icon cho từng loại đơn
const typeIcon: Record<AppType, React.ReactNode> = {
  Leave: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
  OT: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  Other: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16m-7 6h7" /></svg>,
  Resignation: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>,
  PersonnelChange: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>,
};

// Cấu hình Màu nền cho Icon
...
// Cấu hình Màu huy hiệu Trạng thái
...
const mapApiRequest = (r: RequestRecordApi): RequestRecord => {
  ...
  else if (r.requestType === "OTHER") { type = "Other"; typeLabel = "Other Request"; }
  ...
  return { id: r.requestId, type, typeLabel, dateRequested, datesAffected, status, raw: r };
};
...
const Applications: React.FC = () => {
  ...
  // Data States (List Đơn)
  const [requests, setRequests] = useState<RequestRecord[]>([]);
  const [loadingRequests, setLoadingRequests] = useState(true);

  // Form States (Phục vụ Tạo mới / Chỉnh sửa)
  ...
  // 4.1. Tải danh sách đơn từ Backend
  ...
  // 4.2. Khởi chạy Load data khi User đăng nhập
  useEffect(() => {
    loadRequests();
  }, [currentUser?.employeeId]);

  // 4.3. Load Lookup Data khi mở Modal "PersonnelChange"
  useEffect(() => {
  ...
  const handleDropdownItemClick = (type: ModalType) => {
    setModal(type);
    setMenuOpen(false);
    setEditRequestId(null); // Tạo mới nên phải reset ID Edit

    // Nếu xin Nghỉ Phép thì check xem còn bao nhiêu ngày phép
  ...
  const closeModal = () => {
    setModal(null);
    setPcEmployee(null);
    setFormData(BLANK_FORM); // Trả Form về sạch tinh tươm
    setSubmitError(null);
  ...
  const handleEdit = (r: RequestRecord) => {
    ...
    if (r.type === "Leave") {
      ...
      if (match) { leaveType = match[1]; parsedReason = match[2]; }
    } else if (r.type === "OT") {
      const match = parsedReason.match(/^(.*?) - (.*?) \| (.*)$/);
      if (match) { otStartTime = match[1]; otEndTime = match[2]; parsedReason = match[3]; }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // -- Phân nhánh Validate và Build DTO theo loại Modal --
      if (modal === "Leave") {
        if (!formData.startDate || !formData.endDate) throw new Error("Please select start and end dates.");
        dto = { employeeId: currentUser.employeeId, requestType: "LEAVE", startDate: formData.startDate, endDate: formData.endDate, reason: `[${formData.leaveType}] ${formData.reason}` };
      } else if (modal === "OT") {
        if (!formData.otDate || !formData.otStartTime || !formData.otEndTime) throw new Error("Please fill in all OT date and time fields.");
        dto = { employeeId: currentUser.employeeId, requestType: "OT", startDate: formData.otDate, endDate: formData.otDate, reason: `${formData.otStartTime} - ${formData.otEndTime} | ${formData.reason}` };
      } else if (modal === "Other") {
        if (!formData.otherDate || !formData.reason) throw new Error("Please fill in the date and reason for the request.");
        dto = { employeeId: currentUser.employeeId, requestType: "OTHER", startDate: formData.otherDate, endDate: formData.otherDate, reason: formData.reason };
        ...
        await offboardingService.createResignation(currentUser.employeeId, { type: "RESIGNATION", reason: formData.reason, expectedLastDay: formData.startDate });
  ...
  return (
    <div className="flex flex-col pb-10 max-w-7xl mx-auto w-full space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div><h1 className="text-[28px] font-bold text-[#1a1c21] tracking-tight">Application Management</h1></div>
        <div className="relative" ref={menuRef}>
          <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-center space-x-2 px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-lg font-medium text-sm shadow-sm transition-all">
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white border border-[#e2e8f0] rounded-xl shadow-lg z-10 overflow-hidden">
              <button onClick={() => handleDropdownItemClick("Leave")} className="w-full text-left px-4 py-2.5 text-sm font-semibold text-[#0f172a] hover:bg-[#f8fafc] transition-colors border-b border-[#f1f5f9]">Leave Application</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
            ...
            <div className="flex justify-between items-center px-6 py-5 border-b border-[#e2e8f0]">
              <div>
                <h3 className="text-lg font-bold text-[#0f172a]">{editRequestId ? "Edit" : "New"} {modalTitle[modal]}</h3>
                <p className="text-sm text-[#64748b] mt-0.5">
                  {modal === "Leave" && "Submit a leave request for approval."}
                  {modal === "OT" && "Register your overtime hours for approval."}
                  {modal === "Other" && "Submit a general request for approval."}
              ...
              <button onClick={closeModal} className="text-[#94a3b8] hover:text-[#64748b] transition-colors"><svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              {modal === "Leave" && <LeaveModalContent leaveBalanceLoading={leaveBalanceLoading} leaveBalance={leaveBalance} formData={formData} setFormData={setFormData} />}
              ...
              {modal === "PersonnelChange" && <PersonnelChangeModalContent formData={formData} setFormData={setFormData} pcEmployee={pcEmployee} setPcEmployee={setPcEmployee} departments={departments} positions={positions} />}
            </div>

            {/* Modal Footer */}
            <div className="flex justify-end gap-3 px-6 py-4 bg-[#f8fafc] border-t border-[#e2e8f0]">
              <button onClick={closeModal} className="px-5 py-2 rounded-lg border border-[#e2e8f0] text-sm font-semibold text-[#334155] hover:bg-gray-100 transition-colors">Cancel</button>
              <button onClick={handleSubmit} disabled={submitting} className="px-5 py-2 rounded-lg bg-[#0d9488] hover:bg-[#0f766e] disabled:opacity-50 text-white text-sm font-semibold shadow-sm transition-colors">
