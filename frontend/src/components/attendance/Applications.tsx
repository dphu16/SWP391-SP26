...
import { getMyRequests, createRequest, updateRequest, deleteRequest, getLeaveBalance, type RequestRecord as RequestRecordApi, type CreateRequestDTO, type LeaveBalanceResponse } from "../../services/requestService";

// ── Types ──────────────────────────────────────────────────────────────────
type AppStatus = "Pending" | "Approved" | "Rejected";
type AppType = "Leave" | "OT" | "Other";
type ModalType = "Leave" | "OT" | "Other" | null;
...
const Applications: React.FC = () => {
    ...
    const closeModal = () => {
        setModal(null);
        setFormData({ leaveType: "Annual Leave", startDate: "", endDate: "", reason: "", otDate: "", otStartTime: "", otEndTime: "", otherDate: "" });
        setSubmitError(null);
    ...
    const handleDropdownItemClick = (type: ModalType) => {
        ...
        setMenuOpen(false);
        setEditRequestId(null);
        // Fetch leave balance when Leave modal opens
                ...
                <div className="relative" ref={menuRef}>
                    ...
                    {menuOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-white border border-[#e2e8f0] rounded-xl shadow-lg z-10 overflow-hidden">
                            ...
                            <button onClick={() => handleDropdownItemClick("Other")} className="w-full text-left px-4 py-2.5 text-sm font-semibold text-[#0f172a] hover:bg-[#f8fafc] transition-colors">
                                Other Request
                            </button>
                        </div>
                    )}
                        ...
                        <div className="flex justify-between items-center px-6 py-5 border-b border-[#e2e8f0]">
                            <div>
                                <h3 className="text-lg font-bold text-[#0f172a]">
                                    {editRequestId ? "Edit" : "New"} {modal === "Leave" ? "Leave Application" : modal === "OT" ? "OT Application" : "Other Request"}
                                </h3>
                        ...
                        <div className="px-6 py-5 space-y-4">
                            ...
                            {modal === "Leave" && (
                                <>
                                    {/* Leave Balance Card */}
                                    {leaveBalanceLoading ? (
                                        <div className="p-3 bg-[#f8fafc] border border-[#e2e8f0] rounded-lg text-sm text-[#64748b]">Loading leave balance...</div>
                                    ) : leaveBalance ? (
                                        <div className="p-4 bg-gradient-to-r from-[#f0fdf4] to-[#ecfdf5] border border-[#86efac] rounded-xl">
                                            <p className="text-xs font-bold text-[#15803d] uppercase tracking-wider mb-2">Leave Balance {leaveBalance.year}</p>
                                            <div className="flex items-center gap-4">
                                                <div className="flex-1 text-center">
                                                    <p className="text-2xl font-bold text-[#0f766e]">{leaveBalance.annualLeaveTotal - leaveBalance.annualLeaveUsed}</p>
                                                    <p className="text-[10px] text-[#64748b] font-medium">Annual Leave Left</p>
                                                </div>
                                                <div className="w-px h-10 bg-[#86efac]" />
                                                <div className="flex-1 text-center">
                                                    <p className="text-2xl font-bold text-[#64748b]">{leaveBalance.annualLeaveUsed}</p>
                                                    <p className="text-[10px] text-[#64748b] font-medium">Annual Used</p>
                                                </div>
                                                <div className="w-px h-10 bg-[#86efac]" />
                                                <div className="flex-1 text-center">
                                                    <p className="text-2xl font-bold text-[#64748b]">{leaveBalance.sickLeaveUsed}</p>
