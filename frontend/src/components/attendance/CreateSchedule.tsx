...
const EmployeeSelector: React.FC<EmployeeSelectorProps> = ({ selectedEmployee, onSelect }) => {
    ...
    return (
        <div ref={ref} className="relative">
            <label className="block text-sm font-semibold text-[#374151] mb-2">Nhân viên</label>
            {selectedEmployee && !open ? (
                ...
                <input type="text" autoFocus={open} value={query} placeholder="Tìm theo tên hoặc mã nhân viên..."
            ...
            {open && (
                <div className="absolute z-30 mt-1 w-full bg-white border border-[#e2e8f0] rounded-xl shadow-lg overflow-hidden">
                    <div className="px-3 py-2 border-b border-[#f1f5f9]">
                        <input type="text" autoFocus value={query} placeholder="Tìm theo tên hoặc mã nhân viên..."
                            onChange={e => setQuery(e.target.value)}
                            className="w-full text-sm text-[#0f172a] focus:outline-none placeholder-[#94a3b8]" />
                    </div>
                    <div className="max-h-56 overflow-y-auto">
                        {loading && <p className="px-4 py-3 text-sm text-[#94a3b8]">Đang tải...</p>}
                        {!loading && employees.length === 0 && <p className="px-4 py-3 text-sm text-[#94a3b8]">Không tìm thấy.</p>}
    ...
    return (
        <div>
            ...
            <div className="relative mb-2">
                ...
                <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Tìm nhân viên..."
            ...
            <div className="border border-[#e2e8f0] rounded-xl overflow-hidden max-h-64 overflow-y-auto">
                {loading && <p className="px-4 py-3 text-sm text-[#94a3b8]">Đang tải...</p>}
                {!loading && employees.length === 0 && <p className="px-4 py-3 text-sm text-[#94a3b8]">Không tìm thấy nhân viên.</p>}
...
const ResultBanner: React.FC<{ count: number; onClose: () => void }> = ({ count, onClose }) => (
    <div className="flex items-center gap-3 px-4 py-3 bg-green-50 border border-green-200 rounded-xl text-sm text-green-800 mb-6">
        ...
        <span className="font-semibold">Thành công! Đã tạo {count} lịch làm việc.</span>
...
const CreateSchedule: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (tab === "single") {
                if (!selectedEmployee) throw new Error("Vui lòng chọn nhân viên.");
                if (!singleShiftId) throw new Error("Vui lòng chọn ca làm việc.");
                await createSchedule({ employeeId: selectedEmployee.id, date: singleDate, shiftId: singleShiftId });
                setSuccessCount(1);

            } else if (tab === "bulk") {
                if (selectedBulkIds.size === 0) throw new Error("Vui lòng chọn ít nhất 1 nhân viên.");
                if (!bulkShiftId) throw new Error("Vui lòng chọn ca làm việc.");
    ...
    const ShiftSelector = ({ value, onChange }: { value: string; onChange: (id: string) => void }) => (
        <div>
            <label className="block text-sm font-semibold text-[#374151] mb-3">Chọn ca làm việc</label>
            {shiftsLoading
                ? <p className="text-sm text-[#94a3b8]">Đang tải danh sách ca...</p>
                : shifts.length === 0
                    ? <p className="text-sm text-[#94a3b8]">Chưa có dữ liệu ca trong DB.</p>
    ...
    return (
        <div className="flex flex-col pb-10 max-w-3xl">
            {/* Header */}
            <div className="mb-6">
                <h1 className="text-[28px] font-bold text-[#1a1c21] tracking-tight">Tạo Ca Làm Việc</h1>
                <p className="text-[#64748b] text-[15px] mt-1">Xếp lịch đơn, hàng loạt hoặc sao chép từ tháng trước.</p>
            </div>

            {/* Tabs */}
            <div className="flex items-center gap-2 mb-6 p-1.5 bg-[#f8fafc] border border-[#e2e8f0] rounded-2xl w-fit">
                {tabBtn("single", "Tạo đơn lẻ", "📅")}
                {tabBtn("bulk", "Tạo hàng loạt", "⚡")}
                {tabBtn("clone", "Copy tháng trước", "📋")}
            </div>
                    ...
                    <>
                        ...
                        {selectedEmployee && (
                            <>
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-[#374151] mb-2">Ngày làm việc</label>
                ...
                {tab === "bulk" && (
                    <>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-[#374151] mb-2">Từ ngày</label>
                                <input type="date" value={bulkStart} onChange={e => setBulkStart(e.target.value)}
                                    className="w-full px-4 py-2.5 rounded-xl border border-[#e2e8f0] text-[#0f172a] text-sm focus:outline-none focus:ring-2 focus:ring-[#0d9488]/40 focus:border-[#0d9488] transition-all" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-[#374151] mb-2">Đến ngày</label>
                                <input type="date" value={bulkEnd} onChange={e => setBulkEnd(e.target.value)}
                                    className="w-full px-4 py-2.5 rounded-xl border border-[#e2e8f0] text-[#0f172a] text-sm focus:outline-none focus:ring-2 focus:ring-[#0d9488]/40 focus:border-[#0d9488] transition-all" />
                            </div>
                        </div>

                        <div className="flex items-center gap-2 px-3 py-2 bg-[#f0fdf4] rounded-lg border border-green-100 text-xs text-green-700">
                            <span>ℹ️</span>
                            <span>Chủ Nhật và các ngày đã có lịch sẽ tự động bị bỏ qua.</span>
                        </div>
                            ...
                            <>
                                ...
                                <div className="p-4 bg-[#f8fafc] rounded-xl border border-[#e2e8f0]">
                                    <p className="text-sm text-[#475569] leading-relaxed">
                                        Sao chép toàn bộ lịch làm việc từ <strong>tháng trước</strong> sang tháng bạn chọn.
                                        Ngày <strong>Chủ Nhật</strong> và ngày <strong>đã có lịch</strong> sẽ được bỏ qua.
                                    </p>
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-[#374151] mb-3">Tháng đích</label>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs text-[#94a3b8] mb-1.5 font-medium">Tháng</label>
                                        ...
                                        <div>
                                            <label className="block text-xs text-[#94a3b8] mb-1.5 font-medium">Năm</label>
                        ...
                        <button onClick={handleSubmit} disabled={submitting}
                            ...
                            {submitting ? (
                                <>
                                    ...
                                    Đang xử lý {tab === "bulk" ? `${selectedBulkIds.size} nhân viên` : ""}...
                                </>
                            ) : tab === "single" ? "📅 Tạo lịch"
                                : tab === "bulk" ? `⚡ Tạo hàng loạt${selectedBulkIds.size > 0 ? ` (${selectedBulkIds.size} NV)` : ""}`
                                    : "📋 Copy tháng trước"
                            }
                        </button>
                    </div>
