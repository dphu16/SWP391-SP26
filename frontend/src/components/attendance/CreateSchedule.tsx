...
const Avatar: React.FC<{ name: string; size?: string }> = ({
  ...
  return (
    <div
      className={`${size} rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${c}`}
    >
      {initials(name)}
    </div>
  );
...
const EmployeeSelector: React.FC<EmployeeSelectorProps> = ({
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await apiClient.get<PageResponse<Employee>>(
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-[#64748b] truncate">
              {selectedEmployee.employeeCode} · {selectedEmployee.deptName}
            </div>
          </div>
          <svg
      ...
      {open && (
        <div className="absolute z-30 mt-1 w-full bg-white border border-[#e2e8f0] rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="max-h-56 overflow-y-auto">
            {loading && (
              <p className="px-4 py-3 text-sm text-[#94a3b8]">Loading...</p>
            )}
...
const EmployeeMultiSelector: React.FC<EmployeeMultiSelectorProps> = ({
  selectedIds,
  onToggle,
  onSelectAll,
  onDeselectAll,
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() =>
              allSelected ? onDeselectAll() : onSelectAll(employees)
            }
 ...
 ${checked ? "bg-[#f0fdf4]" : "hover:bg-[#f8fafc]"}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`font-semibold text-sm truncate ${checked ? "text-[#0f766e]" : "text-[#0f172a]"}`}
                  >
                    {emp.fullName}
                  </div>
                  <div className="text-xs text-[#64748b] truncate">
                ...
                {checked && (
                  <svg
                    className="w-4 h-4 text-[#0d9488] flex-shrink-0"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
...
// ─── Result Banner ──────────────────────────────────────────────────────────────
const ResultBanner: React.FC<{ count: number; onClose: () => void }> = ({
  count,
  onClose,
}) => (
    ...
    try {
      if (tab === "single") {
        ...
        if (selectedBulkIds.size === 0)
          throw new Error("Please select at least 1 employee.");
        ...
        if (!selectedEmployee) throw new Error("Please select an employee.");
        const result = await cloneScheduleFromPreviousMonth(
          selectedEmployee.id,
          cloneTargetMonth,
          cloneTargetYear,
        );
            ...
            {selectedBulkIds.size > 0 && (
              <div className="flex flex-wrap gap-2">
                {Array.from(selectedBulkEmployees.values()).map((emp) => (
                  <span
                    key={emp.id}
                    className="flex items-center gap-1.5 px-2.5 py-1 bg-[#f0fdf4] border border-[#86efac] rounded-full text-xs font-semibold text-[#15803d]"
                  >
                    {emp.fullName}
                    <button
                      type="button"
                      onClick={() => toggleBulkEmployee(emp)}
                      className="text-[#15803d] hover:text-red-500 font-bold leading-none"
                    >
                      ×
                    </button>
                  </span>
              ...
              <>
                ...
                <div className="p-4 bg-[#f8fafc] rounded-xl border border-[#e2e8f0]">
                  <p className="text-sm text-[#475569] leading-relaxed">
                    Clone all schedules from the <strong>previous month</strong>{" "}
                    to your target month.
                    <strong>Sundays</strong> and dates that{" "}
                    <strong>already have schedules</strong> will be skipped.
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-[#374151] mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setCloneTargetMonth(Number(e.target.value))
                        }
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setCloneTargetYear(Number(e.target.value))
                        }
                        className="w-full px-4 py-2.5 rounded-xl border border-[#e2e8f0] text-[#0f172a] text-sm focus:outline-none focus:ring-2 focus:ring-[#0d9488]/40 focus:border-[#0d9488] transition-all bg-white"
                      >
                        {[
                  ...
                  <div className="mt-3 flex items-center gap-2 text-sm text-[#64748b]">
                    <span className="text-[#0d9488] font-semibold">
                      {
                        MONTH_NAMES[
                          cloneTargetMonth === 1 ? 11 : cloneTargetMonth - 2
                        ]
                      }{" "}
                      {cloneTargetMonth === 1
                        ? cloneTargetYear - 1
                    ...
                    <span className="font-semibold text-[#0f172a]">
                      {MONTH_NAMES[cloneTargetMonth - 1]} {cloneTargetYear}
                    </span>
                  </div>
                </div>
              </>
            ...
            >
              {submitting ? (
                <>
                  ...
                  Processing{" "}
                  {tab === "bulk" ? `${selectedBulkIds.size} employee(s)` : ""}
                  ...
                </>
              ) : tab === "single" ? (
                "📅 Create Schedule"
