...
// ============================================================================
// KHU VỰC 2: CÁC COMPONENT CON (SUB-COMPONENTS)
// ============================================================================
...
// ─── 2.2. Chọn 1 Nhân Viên (Dành cho Single & Clone Tab) ────────────────────
...
// ─── 2.3. Chọn Nhiều Nhân Viên (Dành cho Bulk Tab) ───────────────────────────
        ...
        <div>
            <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                    ...
                    {selectedIds.size > 0 && (
                        <button type="button" onClick={onDeselectAll} className="text-xs font-semibold text-[#ef4444] hover:underline transition-all">
                            Clear
                        </button>
                    )}
                </div>
...
// ─── 2.4. Các UI Elements Phụ (ShiftCard, Banner) ────────────────────────────
...
// ─── 2.5. Modal Tạo Ca Làm Việc Mới (Create Shift Modal) ──────────────────────
...
const CreateShiftModal: React.FC<CreateShiftModalProps> = ({ onClose, onCreated }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            onCreated(created);
        } catch (err: any) { setError(err?.response?.data?.message ?? err.message ?? "Failed to create shift."); }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-5" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><span className="text-xl">🕐</span><h2 className="text-[17px] font-bold text-[#0f172a]">Create New Shift</h2></div>
                ...
                {error && (<div className="flex items-start gap-2 px-3 py-2.5 bg-red-50 border border-red-200 rounded-xl text-xs text-red-700"><span className="flex-shrink-0">⚠️</span><span>{error}</span></div>)}
...
// ============================================================================
// KHU VỰC 3: MAIN COMPONENT (Tích hợp State & Logic)
// ============================================================================

const CreateSchedule: React.FC = () => {
    const today = new Date();

    // ─── 3.1. GLOBAL & TABS STATE ────────────────────────────────────────────
    ...
    // --- General Submit (Single, Bulk, Clone) ---
    ...
    // ============================================================================
    // MAIN RENDER (Giữ nguyên 100% style của bạn)
    // ============================================================================
    return (
        <div className="flex flex-col pb-10 max-w-3xl">
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] shadow-sm p-6 space-y-6">
                    ...
                    <>
                        <div className="flex items-center justify-between">
                            ...
                            <button type="button" onClick={() => setShowCreateShiftModal(true)}
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#0d9488] text-white text-xs font-bold hover:bg-[#0f766e] transition-all">
                                <span>+</span> New Shift
                            </button>
                        </div>
                        {shiftDeleteError && (
                                ...
                                {shifts.map(sh => {
                                    ...
                                    return (
                                        <div key={sh.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[#e2e8f0] bg-white hover:border-[#cbd5e1] transition-colors">
                                            ...
                                            <button type="button" onClick={() => handleDeleteShift(sh.id)} disabled={isDeleting}
                            ...
                            <>
                                ...
                                <div className="flex items-end gap-3">
                                    <div className="flex-1">
                                        ...
                                        <select value={manageMonth} onChange={e => setManageMonth(Number(e.target.value))}
                                            className="w-full px-3 py-2.5 rounded-xl border border-[#e2e8f0] text-[#0f172a] text-sm focus:outline-none focus:ring-2 focus:ring-[#0d9488]/40 focus:border-[#0d9488] bg-white transition-all">
                                            {MONTH_NAMES.map((n, i) => <option key={i + 1} value={i + 1}>{n}</option>)}
                                        </select>
                                    </div>
                                    <div className="w-28">
                                        <label className="block text-xs font-semibold text-[#374151] mb-1.5">Year</label>
                                        <select value={manageYear} onChange={e => setManageYear(Number(e.target.value))}
                                            className="w-full px-3 py-2.5 rounded-xl border border-[#e2e8f0] text-[#0f172a] text-sm focus:outline-none focus:ring-2 focus:ring-[#0d9488]/40 focus:border-[#0d9488] bg-white transition-all">
                                            {[today.getFullYear() - 1, today.getFullYear(), today.getFullYear() + 1].map(y => <option key={y} value={y}>{y}</option>)}
                                        </select>
                                    </div>
                                    <button type="button" onClick={loadManageSchedules} disabled={manageLoading}
                                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#0d9488] hover:bg-[#0f766e] disabled:opacity-60 text-white font-semibold text-sm transition-all">
                                ...
                                {!manageLoading && manageSchedules.length > 0 && (
                                    <>
                                        <div className="flex items-center justify-between">
                                            ...
                                            <button type="button" onClick={handleDeleteAllSchedules} disabled={deletingAll || manageSchedules.length === 0}
                                                    ...
                                                    <div key={sch.id} className="flex items-center gap-3 px-4 py-2.5 rounded-xl border border-[#e2e8f0] bg-white hover:border-[#cbd5e1] transition-colors">
                                                        ...
                                                        {updatingScheduleId === sch.id ? (
                                                            <div className="flex-1 min-w-0 pr-4">
                                                                <select value={updatingShiftId} onChange={e => setUpdatingShiftId(e.target.value)}
                                                        ...
                                                        <div className="flex items-center gap-1 flex-shrink-0">
                                                            {updatingScheduleId === sch.id ? (
                                                                <>
                                                                    <button type="button" onClick={() => handleUpdateSchedule(sch.id)} disabled={updatingLoading || !updatingShiftId}
                                                                    ...
                                                                    <button type="button" onClick={() => setUpdatingScheduleId(null)} disabled={updatingLoading}
                                                                        className="px-3 py-1.5 rounded-lg text-xs font-semibold text-[#64748b] border border-[#e2e8f0] hover:bg-[#f1f5f9] disabled:opacity-50 transition-all">Cancel</button>
                                                                </>
                                                            ) : (
                                                                <>
                                                                    <button type="button" onClick={() => { setUpdatingScheduleId(sch.id); setUpdatingShiftId(sch.shift.id); }} disabled={isDeleting}
                                                                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-[#0d9488] border border-[#0d9488]/30 hover:bg-[#0d9488]/10 disabled:opacity-50 transition-all" title="Change Shift">✏️</button>
                                                                    <button type="button" onClick={() => handleDeleteSchedule(sch.id)} disabled={isDeleting}
                                                                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-red-500 border border-red-200 hover:bg-red-50 disabled:opacity-50 transition-all">
                                                                        {isDeleting ? <svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg> : "🗑️"}
                                                                    </button>
                                                                </>
                ...
                {/* ───────────────────────────────────────────────────────── */}
                {/* ── NÚT SUBMIT TỔNG (Dành cho Single, Bulk, Clone) ── */}
                {/* ───────────────────────────────────────────────────────── */}
