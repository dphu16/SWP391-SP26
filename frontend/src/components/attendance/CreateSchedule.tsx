            ...
            >
                ...
                <div className="flex flex-col gap-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2.5 rounded-xl border border-[#e2e8f0] text-[#0f172a] text-sm focus:outline-none focus:ring-2 focus:ring-[#0d9488]/40 focus:border-[#0d9488] transition-all"
                            autoFocus
                        />
                    </div>
                ...
                <div className="flex gap-3 pt-1">
                    <button
                        type="button"
                        onClick={onClose}
                        className="flex-1 px-4 py-2.5 rounded-xl border border-[#e2e8f0] text-[#64748b] font-semibold text-sm hover:bg-[#f8fafc] transition-all"
                    >
                        Cancel
                    </button>
                    <button
                        ...
                        onClick={handleSave}
                        disabled={saving}
                        className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#0d9488] hover:bg-[#0f766e] disabled:opacity-60 text-white font-semibold text-sm shadow-sm transition-all"
                    >
                        {saving ? (
...
const CreateSchedule: React.FC = () => {
    ...
    const loadManageSchedules = useCallback(async () => {
        ...
        try {
            ...
            setManageError(err?.response?.data?.message ?? err.message ?? "Failed to load schedules.");
        } finally {
    ...
    const handleDeleteShift = async (shiftId: string) => {
        ...
        try {
            ...
            setShiftDeleteError(err?.response?.data?.message ?? err.message ?? "Failed to delete shift.");
        } finally {
    ...
    const handleDeleteSchedule = async (scheduleId: string) => {
        ...
        try {
            ...
            setManageError(err?.response?.data?.message ?? err.message ?? "Failed to delete schedule.");
        } finally {
        ...
        <div className="flex flex-col pb-10 max-w-3xl">
            ...
            <div className="bg-white rounded-2xl border border-[#e2e8f0] shadow-sm p-6 space-y-6">
                ...
                {tab === "shifts" && (
                    <>
                        <div className="flex items-center justify-between">
                            <label className="text-sm font-semibold text-[#374151]">All Shifts
                                <span className="ml-2 text-xs font-normal text-[#94a3b8]">({shifts.length})</span>
                            </label>
                            <button
                                type="button"
                                    ...
                                    <>
                                        ...
                                        <div className="flex flex-col gap-2 max-h-80 overflow-y-auto pr-1">
                                            {manageSchedules.map(sch => {
                                                ...
                                                return (
                                                    <div key={sch.id}
                                                        className="flex items-center gap-3 px-4 py-2.5 rounded-xl border border-[#e2e8f0] bg-white hover:border-[#cbd5e1] transition-colors"
                                                    >
                                                        {/* Date badge */}
