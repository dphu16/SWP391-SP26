...
// FIX 1: Removed duplicate LEAVE/OT/OTHER blocks. The function now has a single,
// clean if/else chain with no repeated logic.
...
const ReviewRequests: React.FC = () => {
    ...
    const displayed = displayedAll.slice(
    ...
    // FIX 2: Single return statement. Removed the premature return that was cutting
    // the component short and leaving HRConfirmOffboardingModal unreachable.
    return (
        <div className="flex flex-col pb-10 max-w-7xl mx-auto w-full space-y-6">
            ...
            <div className="bg-white border border-[#e2e8f0] rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left">
                        ...
                        {loading ? (
                          ...
                          {entry.appType}
                                    ...
                                    {/* FIX 3 (cont.): Single "Details" cell — shows both the
                        "View Details" button and the summary text together,
                ...
                {/* FIX 4: Single footer with working pagination (removed the duplicate
            static/disabled footer that appeared after the early return). */}
                <div className="flex justify-between items-center px-5 py-4 border-t border-[#f1f5f9]">
                    <p className="text-sm text-[#64748b]">
                        Showing{" "}
                        <span className="font-bold text-[#0f172a]">
              {displayedAll.length === 0 ? 0 : (currentPage - 1) * ITEMS_PER_PAGE + 1}
            </span>{" "}
                        to{" "}
                        <span className="font-bold text-[#0f172a]">
              {Math.min(currentPage * ITEMS_PER_PAGE, displayedAll.length)}
            </span>{" "}
                        of{" "}
                        <span className="font-bold text-[#0f172a]">{displayedAll.length}</span>{" "}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
                        ...
                        <div className="px-6 py-5 space-y-4">
                            <div>
                                <h4 className="text-sm font-bold text-[#334155] mb-1">
                                    {selectedDetail.appType}
                                </h4>
                            ...
                            <div>
                                ...
                                <div className="p-3 bg-[#f8fafc] rounded-lg border border-[#e2e8f0]">
                                    <p className="text-sm text-[#475569] whitespace-pre-wrap">
                                        {selectedDetail.sub || "No reason provided."}
                                    </p>
                                </div>
                            </div>
                        </div>
