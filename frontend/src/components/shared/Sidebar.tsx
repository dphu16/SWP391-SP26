...
const NavItem: React.FC<NavItemProps> = ({
  ...
  <button
    ...
    `}
  >
    {isActive && !isCollapsed && <span className="active-nav-indicator" />}
...
const Sidebar: React.FC = () => {
  ...
  const fetchScheduleCount = async () => {
    if (hasRole("HR" as UserRole, "MANAGER" as UserRole) && user?.employeeId) {
      try {
        const res = await applicationService.getInterviewByHr(user.employeeId);
        // Filter for scheduled interviews if needed, or count all
        const pendingCount = res.data.filter(i => i.status === "SCHEDULED").length;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchScheduleCount, 5 * 60 * 1000);
  ...
  return (
    ...
    >
      <div
        className={`h-16 flex items-center flex-shrink-0 border-b border-border-light
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3 px-3 space-y-0.5">
        ...
        <NavItem
          ...
          label="Candidate"
          isActive={isPath("/onboarding/hired")}
          isCollapsed={isCollapsed}
          onClick={() => navigate("/onboarding/hired")}
        />
            ...
            >
              ...
              {!isCollapsed && (
                <>
                  ...
                  <span
                    className={`transition-transform duration-200 ${employeesExpanded ? "rotate-180" : ""}`}
                  >
                    {Icons.chevronDown}
                  </span>
                </>
          ...
          >
            ...
            {!isCollapsed && (
              <>
                ...
                <span
                  className={`transition-transform duration-200 ${requestExpanded ? "rotate-180" : ""}`}
                >
                  {Icons.chevronDown}
                </span>
              </>
          ...
          >
            ...
            {!isCollapsed && (
              <>
                ...
                <span
                  className={`transition-transform duration-200 ${attendanceExpanded ? "rotate-180" : ""}`}
                >
                  {Icons.chevronDown}
                </span>
              </>
              ...
              >
                ...
                {!isCollapsed && (
                  <>
                    ...
                    <span
                      className={`transition-transform duration-200 ${recruitmentExpanded ? "rotate-180" : ""}`}
                    >
                      {Icons.chevronDown}
                    </span>
                  </>
                  ...
                  {[
                    ...
                    .map((item) => (
                      <NavItem
                        ...
                        isCollapsed={false}
                        indent
                        badge={item.path === "/recruitment/schedules" ? scheduleCount : undefined}
                        onClick={() => navigate(item.path)}
                      />
                    ))}
                </div>
