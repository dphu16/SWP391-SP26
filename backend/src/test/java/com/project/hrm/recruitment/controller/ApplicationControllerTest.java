...
/**
 * Tests cho {@link ApplicationController}
 * Base URL: /api/app
 *
 * Public endpoint:
 *   POST /api/app/candidate  – không cần auth (candidate tự nộp CV)
 *
 * Protected endpoints:
 *   GET  /api/app/{id}              – authenticated
 *   GET  /api/app/job/{jobId}       – HR only
 *   POST /api/app/date-limit        – HR only
 *   POST /api/app/list/next-stage   – HR only
 *   PUT  /api/app/last-stage/{id}   – HR only
 *   DELETE /api/app/{id}            – HR only
 */
...
class ApplicationControllerTest {
    ...
    @Test
    @DisplayName("GET /api/app/{id} – lấy chi tiết application → 200")
    @WithMockUser
    ...
    @Test
    @DisplayName("GET /api/app/job/{jobId} – không phải HR → 403")
    ...
    @Test
    @DisplayName("POST /api/app/date-limit – không phải HR → 403")
    @WithMockUser(roles = "MANAGER")
