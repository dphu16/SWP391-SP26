...
public class WorkScheduleService {
    ...
    public WorkScheduleResponse createSchedule(WorkScheduleRequest request) {
        ...
        WorkSchedule existing = workScheduleRepository.findByEmployeeIdAndDate(request.getEmployeeId(),
                request.getDate());
        if (existing != null) {
            throw new RuntimeException("A schedule already exists for this employee on " + request.getDate()
    ...
    public List<WorkScheduleResponse> createBulkSchedules(UUID employeeId, LocalDate startDate, LocalDate endDate,
        ...
        if (endDate.isBefore(startDate))
            throw new RuntimeException("End date (" + endDate + ") must not be before start date (" + startDate + ").");
    ...
    public void deleteSchedulesByMonth(UUID employeeId, int month, int year) {
        ...
        if (schedulesToDelete.isEmpty()) {
            throw new RuntimeException(
                    "No schedules found for this employee in " + startDate.getMonth() + " " + year + ".");
        }

        for (WorkSchedule ws : schedulesToDelete) {
            attendanceLogRepository
                    .deleteAll(attendanceLogRepository.findByWorkSchedule_ScheduleId(ws.getScheduleId()));
        }
